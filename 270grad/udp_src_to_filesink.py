...
class Main:
  ...
  def __init__(self):
    ...
    for p_item in range(self.number_of_streams):
      ...
      muxer = gst.element_factory_make("avimux", "avimuxer_%s" % p_item)
      queueb = gst.element_factory_make("queue")

      self.sink_array.append(gst.element_factory_make("filesink", "filesink%s" % p_item))
      self.sink_array[p_item].set_property("location", self.location_path + self.filename + "_stream" + str(p_item) + "_" + str(self.recorded_stream_count) + ".avi")

      # adding the pipleine elements and linking them together
      self.pipeline_array[p_item].add(source, filter,rtpmp4vdepay, filter2, queuea, muxer, queueb, self.sink_array[p_item])
      gst.element_link_many(source, filter,rtpmp4vdepay, filter2, queuea, muxer, queueb, self.sink_array[p_item])
