...
# this pipeline will recieve the stream and save it in a .avi file
# 
# command line input: 
#   's' = start/stop
#   'q' = quit
...
class Main:
  ...
  def init_pipeline(self):
    ...
    for p_item in range(self.number_of_streams):
      ...
      source = gst.element_factory_make("udpsrc","vsource%s" %p_item) 
      ...
      self.sink_array.append(gst.element_factory_make("filesink", "vsink%s" % p_item))
      ...
      self.pipeline_array[p_item].add(source, filter1, rtpmp4vdepay, avimux, queueb, self.sink_array[p_item])
      gst.element_link_many(source, filter1,rtpmp4vdepay, avimux, queueb, self.sink_array[p_item])
